...
unsigned char * CALLBACK ISOgetBuffer_compr(void) {
	return compr_img->buff_raw[compr_img->sector_in_blk] + 12;
}

#ifdef HAVE_CHD
unsigned char * CALLBACK ISOgetBuffer_chd(void) {
...
// uses bcd format
boolean CALLBACK ISOreadTrack(unsigned char *time) {
...
unsigned char* CALLBACK ISOgetBufferSub(int sector) {
	if (pregapOffset && sector >= pregapOffset) {
		sector -= 2 * 75;
